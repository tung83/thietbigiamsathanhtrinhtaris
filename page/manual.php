...
class manual{
    ...
    function breadcrumb(){
        ...
        if(isset($_GET['id'])){
            ...
            $item=$this->db->getOne('manual','id,title,pId');
            $cate=$this->db->where('id',$item['pId'])->getOne('manual_cate','id,title');
            $str.='
            <li><a href="'.myWeb.$this->view.'/'.common::slug($cate['title']).'-p'.$cate['id'].'">'.$cate['title'].'</a></li>
            <li><a href="#">'.$item['title'].'</a></li>';
        }elseif(isset($_GET['pId'])){
            $cate=$this->db->where('id',intval($_GET['pId']))->getOne('manual_cate','id,title');
            $str.='
    ...
    function manual_cate($pId){
        ...
        if($count>0){
            ...
            foreach($list as $item){
                $str.='<li>'.$this->manual_item($item).'</li>';
            }
            $str.='
            </ul>';
        }        
        ...
        if($pId==0){
            ...
            $cate=$this->db->where('id',$pId)->getOne('manual_cate','id,title');            
