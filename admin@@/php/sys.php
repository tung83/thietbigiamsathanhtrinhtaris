...
function mainProcess($db)
{
    $type=$_GET['type'];
    switch($type){
        case 'sys_cate': return sys_cate($db,$type);break;
        case 'sys': return sys($db,$type);break;
        default:return sys_cate($db,$type);break;
    }
}
function sys_cate($db,$type)
{
	$msg='';
    $act='sys';
    $table='sys_cate';
...
function sys($db,$type)
{
	...
	if(isset($_POST["addNew"])) {
		...
		try{
            ...
            if($form->file_chk($_FILES['file'])){
                WideImage::load('file')->resize(125,90, 'fill')->saveToFile(myPath.$file);
                $db->where('id',$recent);
    ...
    $page_head= array(
                    array('#','Danh sách phân phối')
                );
	$str=$form->breadcumb($page_head);
	$str.=$form->message($msg);
    $head_title=array('Tiêu đề','Địa điểm','Loại','Hình ảnh','Hiển thị');
	$str.=$form->table_head($head_title);
