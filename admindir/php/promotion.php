...
function mainProcess()
{
	...
	{
		$sInsert="insert into service(title,sum,content,active,dates) values(";
		$sInsert.="'$title','$sum','$content',$active,'$dates')";
		$test=mysql_query($sInsert);
		$recent=mysql_insert_id();
		if(checkpic($file)==true)
		{
			$newName=reziseImg(221,113,$_FILES["file"]["tmp_name"],$_FILES["file"]["name"],"../images/service/","");
			mysql_query("update service set img='".$newName."' where id=$recent");
		}
		if($test)
		{
	...
	{
		$sUpdate="update service set title='$title',sum='$sum',content='$content'";
	...
	{
		$sql="select * from service where id=".$_POST["idLoad"];
	...
	{
		$sDelete="delete from service where id=".$_POST["idLoad"];
	...
	//end content column
	//paing park here
	$str.="<tr><td colspan=\"8\" align=\"center\" style=\"padding:5px\">&ensp;".adminPaging(10,$count,"main.php?cnht=promotion&")."</td></tr>";
	...
	$str.='<tr>
				<td style="font-weight:bold">Hình ảnh(221x113):</td>
				<td>
	...
	$str.='<tr>
				...
				<td style="padding:5px;" valign="top">
					<img src="../images/service/'.$rowEdit->img.'" border=0 style="border:1px solid red" width="90"/>
				</td>
			</tr>';	
