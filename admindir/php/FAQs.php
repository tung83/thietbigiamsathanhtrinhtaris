...
function mainProcess()
{
	...
	{
		$sInsert="insert into faqs(title,sum,content,active,dates) values(";
		$sInsert.="'$title','$sum','$content',$active,'$dates')";
		$test=mysql_query($sInsert);
		$recent=mysql_insert_id();
		if(checkpic($fileName)==true)
		{
			imageRex(178,133,$_FILES["file"]["tmp_name"],myPath.$fileName);
			mysql_query("update faqs set img='".$fileName."' where id=$recent");
		}
		if($test)
		{
	...
	{
		$sUpdate="update faqs set title='$title',sum='$sum',content='$content'";
	...
	{
		$sql="select * from faqs where id=".$_POST["idLoad"];
	...
	{
		$sDelete="delete from faqs where id=".$_POST["idLoad"];
	...
	//end header column
	//content column
	$s="select * from faqs order by id desc";
	$tab=mysql_query($s);
	$lim=10;
	...
	//end content column
	//paing park here
	$str.="<tr><td colspan=\"8\" align=\"center\" style=\"padding:5px\">&ensp;".adminPaging(10,$count,"main.php?cnht=faqs&")."</td></tr>";
	...
	$str.='<tr>
				<td style="font-weight:bold">Hình ảnh(178x133):</td>
				<td>
