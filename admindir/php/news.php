...
function news()
{
	...
	{
		$sInsert="insert into news(title,sum,content,active,dates,hot) values(";
		$sInsert.="'$title','$sum','$content',$active,'$dates',$hot)";
		$test=mysql_query($sInsert);
		$recent=mysql_insert_id();
		if(checkImg($fileName)==true)
		{
			imageRex(190,142,$_FILES["file"]["tmp_name"],myPath.$fileName)	;
			mysql_query("update news set img='".$fileName."' where id=$recent");
		}
	...
	{
		$sUpdate="update news set title='$title',sum='$sum',content='$content'";
	...
	{
		$sql="select * from news where id=".$_POST["idLoad"];
	...
	{
		$sDelete="delete from news where id=".$_POST["idLoad"];
	...
	//end header column
	//content column
	$s="select * from news order by id desc";
	$tab=mysql_query($s);
	$lim=10;
	...
	$str.="<tr><td colspan=\"8\" align=\"center\" style=\"padding:5px\">
			&ensp;".adminPaging(10,$count,"main.php?cnht=news&")."</td></tr>";
	...
	$str.='<tr>
				...
				<br/><i style="font-weight:normal">Size hình 190x142</i></td>
				<td>
